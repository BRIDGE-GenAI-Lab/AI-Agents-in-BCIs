...
| 24 | `25_semantic_fair_comparison.py` | yes | `run_manifest.json`, `naturalistic_manifest.json` (must be committed and clean) | `output/tables/semantic_fair_comparison_manifest.json` when called with `--build-manifest`; checkpoints under `output/intermediate/runs_semantic_fair/` and `output/tables/semantic_fair_resolver_curves.csv` when called to run. The reviewer-motivated fair-information follow-up: five new arms (`fair:llm_vocab:advisory`, `fair:llm_vocab:enforced`, `fair:hybrid_semantic_gate`, `fair:exact_resolver_gate`, `fair:lexical_resolver_gate`) over the same 200 frozen naturalistic episodes, run AFTER Task 20 was already reported and frozen before any model call |
| 25 | `25b_smoke_semantic_fair_comparison.py` | yes, small | `run_manifest.json` | `output/tables/smoke_semantic_fair.json`. Full-arm live smoke for every new `fair:*` arm on real episodes before the paid run; divergence between arms is a pass condition, not a note |
| 26 | `26_semantic_primary_table.py` | no | `runs_semantic_fair/`, `runs_natural/`, `runs_recal/` checkpoints, `semantic_fair_resolver_curves.csv` | `output/tables/semantic_primary_comparison.csv`. One row per (arm, model) across the five new arms and the two controls they are fair relative to; every enforced arm is threshold-swept before being reported, so a proposal rate is never compared against an operating point |
| 27 | `27_figure_semantic_comparison.py` | no | `semantic_fair_resolver_curves.csv`, `semantic_primary_comparison.csv`, `runs_semantic_fair/` checkpoints | `output/figures/Figure5.pdf` and `.png`. Panel a: the two resolver-plus-gate frontiers overlaid with each model's advisory point and hybrid curve. Panel b: matched-coverage forest across the 10-model panel for the three vocabulary-disclosed arms |
| 28 | `28_semantic_fair_inventory.py` | no | `output/intermediate/runs_semantic_fair/*.parquet` | `output/tables/semantic_fair_dataset_inventory.csv`. The fair-information comparison is a seventh dataset, run outside `17_build_supplement.py`'s pre-specified six-dataset inventory scope, so its run/request/cost totals are tabulated here instead of being stated without a source the number audit can match |
