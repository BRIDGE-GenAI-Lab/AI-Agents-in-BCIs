...
SUPERSEDED = [
    ...
    # "eFigure 4" WAS on this list, from an earlier renumbering that moved it to
    # eFigure 2. The 2026-09-01 figure redesign demoted the calibration figure
    # and eFigure 4 became a real, current name again, at which point this rule
...
def check_reading_copy() -> list[str]:
    """The reading copy has no .md of its own, so the pair loop cannot see it.

    `7_Manuscript_With_Figures.{pdf,docx}` is the manuscript with every figure
    placed above its legend. It is composed from TWO sources rather than one,
    so it has no sibling markdown and drops straight out of the pairing above.
    Without this it would be the one item in the packet nothing checked, while
    the closing line still said the packet was verified.

    Checked on content, not mtime: every substantial paragraph of BOTH sources
    must appear in the .docx. The PDF is checked only for existence and
    freshness, because extracting its text means reflowing a two-column-free
    but hyphenated LaTeX render and comparing that to markdown, which produces
    false alarms rather than caught staleness.
    """
    docx = PACKET / "7_Manuscript_With_Figures.docx"
    pdf = PACKET / "7_Manuscript_With_Figures.pdf"
    sources = [PACKET / "3_Manuscript.md", PACKET / "5_Figure_Legends.md"]

    problems = []
    for f in (docx, pdf):
        if not f.is_file():
            problems.append(f"READING COPY MISSING: {f.name}. Run build_packet.py.")
    if problems:
        return problems

    haystack = _normalise(docx_text(docx, drop_superscripts=True))
    total = 0
    for src in sources:
        paras = md_paragraphs(src)
        total += len(paras)
        absent = [p for p in paras if p not in haystack]
        if absent:
            problems.append(
                f"CONTENT MISSING from {docx.name}: {len(absent)} of {len(paras)} "
                f"substantial paragraph(s) of {src.name} do not appear in it. It "
                f"was not rebuilt from the current sources. First missing: "
                f"{absent[0][:110]!r}"
            )
    newest_src = max(f.stat().st_mtime for f in sources)
    if pdf.stat().st_mtime < newest_src:
        problems.append(f"STALE: {pdf.name} is older than {sources[0].name} or "
                        f"{sources[1].name}. Rebuild it.")
    if not problems:
        print(f"  ok  7_Manuscript_With_Figures.pdf/.docx  "
              f"({total} paragraphs matched across 2 sources)")
    return problems
...
def main() -> int:
    ...
    problems.extend(check_reading_copy())
    ...
    if problems:
        ...
        print(f"\n{len(problems)} problem(s). The .docx is the file the journal reads.",
              file=sys.stderr)
        return 1
    print(f"\n{len(pairs)} pair(s) checked, plus the reading copy and the figure "
          f"directory: every .docx is at least as new as its .md, carries no "
          f"superseded string, and contains every substantial paragraph of its "
          f"source.")
    return 0
