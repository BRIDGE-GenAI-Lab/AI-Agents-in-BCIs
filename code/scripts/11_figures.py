...
DATA ROUTING IS THE POINT OF THIS FILE, so it is stated first.
  ...
  Figure 1b (panel) The risk-coverage frontier on the PRINCIPAL data. The
            ...
            POINT it is (no knob to sweep). `draw_risk_coverage()` draws this
            panel onto a caller-supplied axis; `23_figure1_architecture.py`
            composes it with the panel-a schematic into Figure 1. This file
            also still builds it standalone as `figure1_risk_coverage_frontier`.
  ...
  eFigure 3 The caution-wording battery, PILOT: unfaithful execution against
            parse failure, which separates "the wording worked" from "the
            wording broke the agent". Demoted from Figure 3.
  eFigure 4 Calibration of the reconstructed decoder confidence: per-study
            reliability, and cross-study transport. Neither depends on any
            model run. Demoted from Figure 4.
  Figure 4  The naturalistic benchmark: five models x three arms against the
            deterministic lexical reference paired with the same gate.
            Renumbered from Figure 5.
...
NATURAL_RUNS = INTERMEDIATE / "runs_natural"       # Figure 4 (naturalistic) only
...
DARK = "#1F2A44"   # non-policy reference tone: random gate, exact-matcher line


def fmt3(v: float) -> str:
    """Three-decimal formatting that never prints a signed zero.

    Values a hair below zero (e.g. a bootstrap CI bound of -2.5e-06) round to
    "-0.000" under plain `.3f`, which reads as a negative result that is not
    there.
    """
    s = f"{v:.3f}"
...
# --------------------------------------------------------------------------
# Figure 1b -- risk-coverage frontier, PRINCIPAL data
# --------------------------------------------------------------------------
def draw_risk_coverage(ax, d, cells, *, legend_bbox_to_anchor=(0.5, -0.24)):
    """The paper's central claim, drawn onto a caller-supplied axis.
    ...
    Takes an axis rather than owning a figure so `23_figure1_architecture.py`
    can compose this panel with the panel-a schematic without duplicating the
    plot: a copy would diverge from this one the first time either is touched.
    `legend_bbox_to_anchor` defaults to the value `fig1()` has always used, so
    the standalone build is unaffected; the two-panel composition passes its
    own, because the panel's axes-fraction height is different there and the
    default anchor would either crowd the x-axis or run off the figure.
    Returns the summary dict `fig1()` writes into `figure_checks.json`.
    """
    ...
    ax.text(0.62, 0.050, "Below the deterministic gate", fontsize=7, color=LABEL,
            ha="center", zorder=6)
    ax.text(0.62, 0.030, f"largest excursion into it by any arm: {abs(worst):.3f}",
            fontsize=6.5, color=MUTED, ha="center", zorder=6)
    ...
    for _, r in mc.iterrows():
        ...
        ax.annotate(SHORT.get(r["model"], r["model"]),
                    ...
                    ha=aligns.get(r["model"], "right"), fontsize=6.5,
    ...
    if len(cl) == 2:
        ax.annotate("DeepSeek v4 Flash\nGPT-5.6 Luna",
                    ...
                    textcoords="offset points", xytext=(-36, 26), ha="right",
                    fontsize=6.5, color=LABEL, zorder=8,
    ...
    ax.set_ylabel("Unfaithful execution among actions taken", fontsize=7)
    ...
    ax.legend(handles=handles, loc="lower center", bbox_to_anchor=legend_bbox_to_anchor,
              ncol=2, fontsize=6.5, frameon=False, handletextpad=0.5, columnspacing=1.6)
    return {"worst_excursion": worst, "excursions": excursions,
            "random_gate_risk": random_risk}


def fig1(d, cells):
    """Standalone build of the risk-coverage frontier panel.

    `23_figure1_architecture.py` calls `draw_risk_coverage()` directly to
    compose the same panel with the architecture schematic as Figure 1; this
    function keeps it available on its own as `figure1_risk_coverage_frontier`.
    """
    fig, ax = plt.subplots(figsize=(WIDTH_DOUBLE, 6.3))
    fig.subplots_adjust(bottom=0.20, top=0.97)
    result = draw_risk_coverage(ax, d, cells)
    save(fig, "figure1_risk_coverage_frontier", FIGS)
    return result
...
def fig2():
    ...
    mc = pd.read_csv(TABLES / "primary_matched_coverage.csv")
    groups = [("decoder_confidence", "Reconstructed decoder\nconfidence in the prompt"),
              ("none", "No uncertainty\nin the prompt")]
    ...
    # Column layout in FIGURE fraction, not axes fraction: the coverage and
    # difference columns are gutters between/after the two panels, and an
    # axes-fraction offset (the pre-redesign approach) rescales with whatever
    # width the panel happens to have. A blended transform anchors the text
    # column at a fixed figure-fraction x regardless of panel width, so this
    # layout does not quietly break the next time a width changes.
    fig, axes = plt.subplots(1, 2, figsize=(WIDTH_DOUBLE, 5.2), sharey=True,
                             gridspec_kw={"width_ratios": [1.0, 1.0], "wspace": 0.55})
    fig.subplots_adjust(left=0.235, right=0.855, bottom=0.20, top=0.90)
    from matplotlib.transforms import blended_transform_factory
    trans_a = blended_transform_factory(fig.transFigure, axes[0].transData)
    trans_b = blended_transform_factory(fig.transFigure, axes[1].transData)
    ...
    ax.scatter(rows["nonllm_risk_at_matched_coverage"], ypos, s=70, color=BLUE,
               edgecolors="white", linewidths=0.8, zorder=5)
    ax.scatter(rows["risk_among_acted"], ypos, s=38, color=SALMON, marker="D",
               edgecolors="white", linewidths=0.8, zorder=6)
    ax.set_xlim(0.08, 0.34)
    ax.set_xlabel("Unfaithful execution\namong actions taken", fontsize=7)
    ax.set_yticks(ypos)
    ...
    for yh, header in headers:
        ax.text(0.015, yh, header, transform=blended_transform_factory(
                    fig.transFigure, ax.transData),
                ha="left", va="center", fontsize=7, fontweight="bold", color=LABEL)
    panel_label(ax, "a", x=-0.62, y=1.06)

    # Coverage column, in the gutter between the two panels.
    ax.text(0.495, ypos.max() + 0.75, "Coverage", transform=trans_a,
            ha="center", va="center", fontsize=6.5, color=LABEL)
    for yi, (_, r) in zip(ypos, rows.iterrows()):
        ax.text(0.495, yi, f"{r['coverage']:.3f}", transform=trans_a,
                ha="center", va="center", fontsize=6.5, color=MUTED)

    # --- b: matched gap with its participant-clustered 95% CI --------------
    ax = axes[1]
    ax.axvline(0.0, color=RULE, ls="--", lw=0.7, zorder=1)
    for yi, (_, r) in zip(ypos, rows.iterrows()):
        ...
        ax.scatter([r["matched_gap"]], [yi], s=42, color=col, marker="D",
                   edgecolors="white", linewidths=0.8, zorder=6)
    ax.set_xlim(-0.028, 0.20)
    ax.set_xlabel("Matched-coverage difference\nin unfaithful execution\n"
                  "(arm minus gate; positive favours gate)", fontsize=7)
    ax.tick_params(labelsize=6)
    # Point estimate and interval on two lines: at journal column width there
    # is not room for "0.087 (0.042 to 0.149)" on one line at a legible size.
    ax.text(0.875, ypos.max() + 0.75, "Difference\n(95% CI)", transform=trans_b,
            ha="center", va="center", fontsize=6.5, color=LABEL, linespacing=1.3)
    for yi, (_, r) in zip(ypos, rows.iterrows()):
        ax.text(0.875, yi,
                f"{fmt3(r['matched_gap'])}\n({fmt3(r['matched_gap_lo'])} to "
                f"{fmt3(r['matched_gap_hi'])})",
                transform=trans_b, ha="center", va="center",
                fontsize=6, color=MUTED, linespacing=1.3)
    panel_label(ax, "b", x=-0.12, y=1.06)
    # Left-anchored, not right-anchored. Right-anchoring at 0.98 put the
    # string's LEFT edge on the panel's left spine, so the sentence began
    # flush against the axis line. Anchoring left leaves the slack on the
    # side that has room.
    ax.text(0.02, 0.017, "0 of 10 agent conditions outperformed the gate",
            transform=ax.transAxes, ha="left", va="bottom",
            fontsize=6, color=LABEL,
            # The zero reference line runs the full height of the panel, so
            # the sentence crosses it. An opaque backing keeps the glyphs
            # readable without moving the line or the text.
    ...
    fig.legend(handles=handles, loc="lower center", bbox_to_anchor=(0.545, 0.0),
               ncol=2, fontsize=6.5, frameon=False, handletextpad=0.5, columnspacing=1.6)
    save(fig, "Figure2", FIGS)
    return rows


# --------------------------------------------------------------------------
# eFigure 3 -- caution-wording battery, PILOT (exploratory) data. Demoted
# from Figure 3.
# --------------------------------------------------------------------------
def fig3():
    ...
    fig, axes = plt.subplots(1, 3, figsize=(WIDTH_DOUBLE, 4.6), sharey=True,
                             gridspec_kw={"width_ratios": [1.0, 1.0, 1.0],
                                          "wspace": 0.26})
    # `left` is far smaller than before: the y-axis now carries short w0..w11
    # tags rather than the full wording text, which is what consumed the
    # margin. The verbatim wordings live in the figure legend instead.
    fig.subplots_adjust(left=0.09, bottom=0.24)
    ...
    for k, (col, xlabel, xlim, ref, ref_label, val_side, ref_side) in enumerate(specs):
        ...
        span = xlim[1] - xlim[0]
        if ref is not None:
            ax.axvline(ref, color=RULE, ls="--" if col == "unsafe" else ":",
                       lw=0.8, zorder=1)
        ...
        panel_label(ax, "abc"[k], x=-0.30 if k == 0 else -0.10, y=1.02)

    # Wording text moved to the figure legend: the full strings consumed
    # roughly half the figure width at journal column size. w0-w11 match the
    # `wording_idx` values eTable 2 and this figure's legend use for the same
    # rows.
    axes[0].set_yticks(y)
    axes[0].set_yticklabels(
        [f"w{int(r.wording_idx)}" + ("  *" if int(r.wording_idx) in sig else "")
...
def fig4():
    ...
    # The pooled ("overall") row is excluded from panel a's bars; its value
    # (0.012) is reported in the figure legend, not recomputed here.
    ...
    # right=0.955 clipped the colourbar's own label off the canvas. save()
    # deliberately does not use bbox_inches="tight" (it changes the width),
    # so anything past the figure edge is simply lost.
    fig.subplots_adjust(left=0.085, right=0.895, bottom=0.17, top=0.88)
    ...
    ax.set_xticks(x)
    ax.set_xticklabels(rel["study"], fontsize=6.5)
    # The axis carries BOTH series and they are not the same quantity: a Brier
    # score is a proper score, not a calibration error, so labelling the shared
    # axis "Calibration error" mislabels the salmon bars.
    ax.set_ylabel("Expected calibration error and Brier score", fontsize=7)
    ...
    # The diagonal is UNDEFINED, not zero, and the Blues ramp bottoms out at
    # white -- so an unpainted diagonal cell reads as a perfect 0.000 transport
    # rather than as a transport that was never made. Paint it off-ramp grey
    # and put a dash in it.
    cmap = plt.get_cmap("Blues").copy()
    cmap.set_bad(color="#E6E8EA")
    im = ax.imshow(np.ma.masked_invalid(M), cmap=cmap, vmin=0, vmax=vmax)
    ax.set_xticks(range(len(studies)))
    ax.set_xticklabels(studies, fontsize=6.5)
    ax.set_yticks(range(len(studies)))
    ax.set_yticklabels(studies, fontsize=6.5)
    ax.set_xlabel("Applied to", fontsize=7)
    ax.set_ylabel("Calibrated on", fontsize=7)
    ...
    cb.set_label("Expected calibration error", fontsize=7)
    cb.ax.tick_params(labelsize=6.5)
    cb.outline.set_visible(False)
    panel_label(ax, "b", x=-0.24, y=1.05)

    save(fig, "eFigure4", FIGS)
    return {"n_studies": len(studies), "worst_transport": vmax}


# --------------------------------------------------------------------------
# Figure 4 -- the naturalistic benchmark. Renumbered from Figure 5.
# --------------------------------------------------------------------------
NATURAL_ARMS = [
    (ADVISORY_NONE, "No uncertainty, advisory", SALMON, "o", 0.24),
    (ADVISORY_DC, "Decoder confidence, advisory", SALMON, "D", 0.0),
    (ENFORCED_DC, "Decoder confidence, enforced", GREY, "s", -0.24),
]
COMPARATORS = {
    "natural_confidence_gate_lexical": "Lexical reference + gate",
...
def fig5():
    ...
    fig, axes = plt.subplots(1, 3, figsize=(WIDTH_DOUBLE, 4.2), sharey=True,
                             gridspec_kw={"wspace": 0.16})
    # left=0.20 clears "DeepSeek v4 Flash", the widest y-tick label, at 7 pt.
    fig.subplots_adjust(left=0.20, bottom=0.28)
    ...
    for k, (col, xlabel, xlim, show_ref) in enumerate(specs):
        ...
        if show_ref:
            for name, label in COMPARATORS.items():
                ...
                ax.axvline(v, color=BLUE, lw=1.1, ls=ls, zorder=2)
        ...
        ax.set_xlim(*xlim)
        ax.set_xlabel(xlabel, fontsize=7, labelpad=6)
        ax.tick_params(labelsize=6.5)
        panel_label(ax, "abc"[k], x=-0.42 if k == 0 else -0.10, y=1.05)
    axes[0].set_yticks([ybase[m] for m in models])
    axes[0].set_yticklabels([SHORT[m] for m in models], fontsize=7)
    ...
    # MUTED, not BLUE. Blue is the deterministic comparator's colour on the
    # LINES, and colouring a sentence with it makes the same hue mean both a
    # data series and a note about it. Every annotation in this figure set is
    # MUTED; only marks carry palette colour.
    ...
    handles += [
        mlines.Line2D([], [], color=BLUE, lw=1.1, ls=(0, (5, 2)),
                      label=f"Lexical reference + gate "
                            f"(coverage {comp['natural_confidence_gate_lexical']['coverage']:.3f})"),
        mlines.Line2D([], [], color=BLUE, lw=1.1, ls=(0, (1, 2.5)),
                      label=f"Exact matcher + gate, no model "
                            f"(coverage {comp['natural_confidence_gate_canonical']['coverage']:.3f})"),
    ]
    fig.legend(handles=handles, loc="lower center", bbox_to_anchor=(0.5, 0.0),
               ncol=2, fontsize=6.5, frameon=False, handletextpad=0.5, columnspacing=1.8)
    save(fig, "Figure4", FIGS)
